...
class DeepONetStyleFNO(nn.Module):
    def __init__(
        self,
        num_modes: Tuple[int, ...],
        in_channel: int,
        ...
        self.condition_mlp = nn.Sequential(
            nn.Linear(condition_embed_dim, condition_embed_dim * 2),
            nn.GELU(),
            nn.Linear(condition_embed_dim * 2, hidden_channel)
        )
    ...
    def forward(self, x: torch.Tensor, condition: torch.Tensor) -> torch.Tensor:
        ...
        Args:
            ...
            condition: [B, num_params] (Physical Parameters)
        """
        # Positional Embedding & Lifting
        x = self.positional_embedding(x) # [B, C+dim, H, W]

        # condition embedding
        cond_embd = self.conditional_embedding(condition)  # [B, condition_embed_dim]
        cond_feat = self.condition_mlp(cond_embd)          # [B, hidden]
        ...
        for layer_idx in range(self.num_layers):
            x = self.fno_blocks(x, index=layer_idx)
        
        # combine condition feature
        cond_feat = cond_feat.view(cond_feat.shape[0], cond_feat.shape[1], *([1]*len(x.shape[2:])))
        x = x * cond_feat
